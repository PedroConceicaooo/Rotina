...
(async () => {
  ...
  const ok = (nome, cond, extra = '') =>
    passos.push(`${cond ? '✓' : '✗'} ${nome}${extra ? ' — ' + extra : ''}`);
  ...
  ok(
    'água soma 250ml',
    (await page.locator('#agua-n').textContent()).trim() === '250 ml',
  ...
  ok(
    'histórico de treino',
    /nos últimos 14 dias/.test(await page.locator('#treino-historico').textContent())
  );
  ...
  await page.waitForTimeout(300);
  ok(
    'estudo registrado no dia',
    (await page.locator('#estudo-n2').textContent()).trim() === '90 min',
  ...
  ok(
    ...
    (await page.locator('#agua-n').textContent()).trim() === '750 ml',
    await page.locator('#agua-n').textContent()
  );
  ...
  ok(
    'sem barra de atualização quando a versão é a mesma',
  ...
  try {
    ...
    ok(
      'barra aparece quando sai versão nova',
      (await page.locator('#barra-atualizacao').count()) === 1
    );
    ...
    ok(
      'dados sobrevivem à atualização',
      (await page.locator('#agua-n').textContent()).trim() === '750 ml',
      await page.locator('#agua-n').textContent()
    );
  } finally {
